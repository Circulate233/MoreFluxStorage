...
import com.circulation.more_flux_storage.network.FluxGuiActionPacket;
...
public class FluxGuiConnectorScreen extends AbstractContainerScreen<FluxGuiConnectorMenu> {
    ...
    protected void init() {
        ...
        customName.setResponder(string ->
            MoreFluxStorageNetwork.CHANNEL.sendToServer(
                FluxGuiActionPacket.setName(connector.getFluxGuiPos(), customName.getValue()))
        );
        ...
        priority.setResponder(string ->
            MoreFluxStorageNetwork.CHANNEL.sendToServer(
                FluxGuiActionPacket.setPriority(connector.getFluxGuiPos(), priority.getValidInt()))
        );
        ...
        limit.setResponder(string ->
            MoreFluxStorageNetwork.CHANNEL.sendToServer(
                FluxGuiActionPacket.setLimit(connector.getFluxGuiPos(), limit.getValidLong()))
        );
    ...
    private void onSwitchClicked(FluxSwitchButton button) {
        ...
        if (button == surgeMode) {
            surgeMode.toggle();
            MoreFluxStorageNetwork.CHANNEL.sendToServer(
                FluxGuiActionPacket.setSurge(connector.getFluxGuiPos(), surgeMode.isChecked()));
        } else if (button == disableLimit) {
            disableLimit.toggle();
            MoreFluxStorageNetwork.CHANNEL.sendToServer(
                FluxGuiActionPacket.setDisableLimit(connector.getFluxGuiPos(), disableLimit.isChecked()));
        } else if (button == chunkLoading) {
            chunkLoading.toggle();
            MoreFluxStorageNetwork.CHANNEL.sendToServer(
                FluxGuiActionPacket.setChunkLoading(connector.getFluxGuiPos(), chunkLoading.isChecked()));
        }
    }
    ...
    public boolean keyPressed(int keyCode, int scanCode, int modifiers) {
        ...
        } else if (keyCode == GLFW.GLFW_KEY_ESCAPE || Minecraft.getInstance().options.keyInventory.isActiveAndMatches(key)) {
            onClose();
            return true;
        }
